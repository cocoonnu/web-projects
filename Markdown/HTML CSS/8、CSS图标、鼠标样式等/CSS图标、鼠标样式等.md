...
2、在你想要的css文件中引入fonts项目 用一下代码 注意路径问题
@font-face {
...
3、伪元素引用用Unicode码 ：去掉&#X 剩下的四个字符 并且要转义\
.arrow-icon::after {
    content: '\e6db';
    font-family: 'iconfont';
}

4、span盒子作为字体图标：直接用第二种font-class方法即可
```
...
应用：将一个盒子变成行内块元素，后面的文字也为行内块元素，则加上vertical-align：middle可让文字居中
