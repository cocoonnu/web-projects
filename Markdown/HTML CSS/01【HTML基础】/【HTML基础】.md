...
**一般img标签需要外层再套一个盒子imgBx**  然后img的属性只需要适应盒子

#### 2.4.6.1 object-fit属性
...
定义一个父类设置宽度，高度，然后图片作为子类，width100%，height100%
...
.goods {
    ...
    /* 行高会继承， 会继承给3个孩子 使子类垂直居中 */
    line-height: 60px;
}
/*目录盒子 如果外盒子还要装其他盒子则需要加浮动*/
.goods ul {
    float: left;
    margin-left: 30px;
}
...
// 后面不用定义input变量 直接 loginForm.username loginForm.password loginForm.submit
```

2、

点击submit后自动发送请求，然后自动跳转为服务端的响应体！可阻止默认行为：`e.preventDefault();`

3、

提交表单：`loginForm.submit();`  且表单里面不能有name='submit'的input
...
req.query:
{ username: 'sdafsfr', password: '123456', submit: '提交' }
```
...
| `submit`   | **定义提交按钮**，提交按钮会把表单数据发送到服务器（**点击自动刷新**） |
| `text`     | 定义单行的输入字段，用户可在其中输入文本，默认宽度为 20 个字符 |
| `range`    | 滑块控件的表单类型 属性：runoob.com/jsref/dom-obj-range.html |
...
**除 type 属性外，`<input>` 、`textarea`标签还有很多其他属性，其常用属性如下：**
...
| `required`  | bool         | required 属性规定必需在提交表单之前填写输入字段。 |
| `placeholder`   | 输入框内提示字符                                             ||
| `input.focus()` | input:focus                                          |当input聚焦时|
| outline:none    | 使输入框不发光                                               ||
| `input:valid` |  |当input的value合法时|
| h5新增属性在后面 | ||
...
`<label>` 标签为 input 元素定义标注（标签）。`for`与`id`对应

**`<label>` 标签用于绑定一个表单元素，当点击 `<label>` 标签时，浏览器就会自动将焦点（光标）转到或者选择对应的表单元素上**，用来增加用户体验。

**语法：**

```html
sex:
<input type="radio" name="sex" id="0" ><label for="0">man</label>
<input type="radio" name="sex" id="1" ><label for="1">woman</label>
```
...
[HTML特殊字符编码大全](https://blog.csdn.net/jack_rose_me/article/details/124603494)
...
对于以上特殊字符 提供了一个标签不需要转义即可使用！

```html
<pre> 标签可定义预格式化的文本。
```

被包围在 <pre> 标签 元素中的文本通常会保留空格和换行符等特殊字符。而文本也会呈现为等宽字体。
...
- 制表符tab在<pre>标签定义的块当中可以起到应有的作用，每个制表符占据8个字符的位置，但并不推荐使用tab，因为在不同的浏览器中，tab的表现形式各不相同。在用<pre>标签格式化的文档段中使用**空格**，可以确保文本正确的水平位置。
- 如果希望使用<pre>标签来定义计算机源代码，比如HTML源代码，请使用**符号实体**来表示特殊字符。一般将<pre>标签与**<code>标签**结合起来使用，以获得更加精确的语义，用于标记页面中需要呈现的源代码。
- 如果想要把某一段**规定好的文本格式**放在HTML中，那么就要利用pre元素的特性。
