...
1、每个网页都会有一个基础的结构标签（骨架标签：4组），页面内容也是在这些基础标签上书写的。
...
2、html 全局属性：https://www.runoob.com/tags/ref-standardattributes.html
...
所有特殊字符参考：https://blog.csdn.net/jack_rose_me/article/details/124603494



### 2.5.3 pre 标签

对于以上特殊字符 提供了一个标签**不需要转义即可使用！**
...
3、表单的组成：在 HTML 中，一个完整的表单通常由 `表单域`、`表单控件`（也称为表单元素）和 `提示信息`  3 个部分构成。
...
> 点击submit后自动发送请求，然后自动跳转为服务端的响应体！阻止默认行为：`e.preventDefault();`
>
> form 表单中 method 的 get 和 post 区别：在 Ajax 里面有笔记
...
| `radio`     | 单选按钮，在一组单选按钮中**，要求它们必须拥有相同的 name**  |
| `reset`     | 重置按钮，重置按钮会清除表单中的所有数据                     |
| `submit`    | **提交按钮**，提交按钮会把表单数据发送到服务器（**点击自动刷新**） |
| `text`      | 定单行的输入字段，用户可在其中输入文本，默认宽度为 20 个字符 |
| **`range`** | **滑块控件**的表单类型，介绍：https://www.runoob.com/jsref/dom-obj-range.html |
...
>以下为基本用法，其实和文本框差不多的作用，唯一的区别就是用户界面是不可见的。
>
...
| 属性名      | 属性值       | 描述                                              |
| ----------- | ------------ | ------------------------------------------------- |
| `name`      | 由用户自定义 | 定义 input 元素的名称，**用于 html 区分**              |
| `value`     | 由用户自定义 | 规定 input 元素的值，也就是提交到服务器的值       |
| `checked`   | checked      | 规定此 input 元素**首次加载时应当被选中**         |
| `maxlength` | 正整数       | 规定输入字段中的字符的最大长度                    |
| `required`  | bool         | required 属性规定必需在提交表单之前填写输入字段。 |
| `placeholder`   | string         |输入框内提示字符|
| `input.focus()` | 函数                                        |使 input 聚焦|
...
作用：  **`<label>` 标签用于绑定一个表单元素，当点击 `<label>` 标签时，浏览器就会自动将焦点（光标）转到或者选择对应的表单元素上**
...
使用场景：在页面中，如果有多个选项让用户选择，并且想要节约页面空间时，我们可以使用 `<select>` 标签控件**定义下拉列表**。
...
在表单元素中，`<textarea>` 标签是用于定义多行文本输入的控件。使用多行文本输入控件，可以输入更多的文字，该控件常用于留言板、评论。
...
button 样式大全：https://uiverse.io/buttons
...
<img src="mark-img/image-20220121173139191.png" style="zoom: 25%;" />

**2、使用 CSS 隐藏 `<iframe>` 滚动条**
