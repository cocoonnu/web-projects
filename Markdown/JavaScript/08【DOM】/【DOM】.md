...
<img src="C:\Users\LENOVO\AppData\Roaming\Typora\typora-user-images\image-20221027113132906.png" alt="image-20221027113132906" style="zoom:150%;" />
...
`innerHTML` 属性能以 HTML 语法设置节点中的内容。没有length

`innerText`、`textContent` 属性只能以纯文本的形式设置节点中的内容。有length
...
`ol.children[index] && ol.children[index].classList.add('current')`
...
(6)通过ClassList.toggle可以为元素切换类; 不存在则添加 存在则删除 
...
标准 W3C 属性，如 `src`、`href`、`title`、`alt` 、`index`等等，只需要**直接打点进行更改即可**。

`oImg.src = 'images/2.jpg';`

**`lis[i].index = i`：动态添加序号**
...
### 7.3.1 创建获取html自定义属性data-*

对于html自定义属性 ，要使用 `setAttribute()` 和 `getAttribute()` 来设置、读取。
```js
var div = document.querySelector('div');

// H5新增！自定义属性必须已data-开头
// 设置自定义属性

// 方法一
div.setAttribute('data-index',12);
...
// 当类名很长时，用驼峰命名法
div.setAttribute('data-index-list',13);
var list = div.dataset.indexList;
console.log(list);
...
function activeLink() {
    ...
    // 移动indicator
    let move = this.index * this.offsetHeight;
...
创建方法二：element.insertAdjacentHTML(position, text);
...
nav.insertAdjacentHTML('beforeend', section);
...
`obj.replaceWith(obj)`：可实现节点的替换

```js
oldBoard.replaceWith(parser.querySelector('.newBoard'));
```
...
[BOM重点核心](https://www.bilibili.com/video/BV1Sy4y1C7ha/?p=246&spm_id_from=pageDriver&vd_source=efb9c3f27a68ec1730be5a5e4e050c4b)
...
![image-20221030163642303](C:\Users\LENOVO\AppData\Roaming\Typora\typora-user-images\image-20221030163642303.png)
...
据说未来会被淘汰，以后用不了了就去官网查一下 **推荐用e.key 和 e.code**
...
`e.stopPropagation()` 方**法用来阻止事件继续传播。** 阻止事件冒泡！！！
...
官方定义：不是每个子节点单独设置监听器，而是其监听器设置在其父节点上，然后利用冒泡原理影响设置每个子节点
...
icon_right.addEventListener('click',function() {
    ...
    if (flag == true)
    {
        ...
        // 执行动画时在回调函数中改回来即可
        bufferMove(focus_ul,-num * focusWidth, function(){flag=true;});
    }
