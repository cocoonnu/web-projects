...
## 6.4 bind()
```js
// bind()：改变this指向并返回一个新函数 不会直接调用! 
var new_fn = fn.bind(f);
new_fn(2);
...
- 构**造函数必须用 new 关键字调用，否则不能正常工作**，正因如此，开发者约定构造函数命名时首字母要大写
...
`hasOwnProperty` 方法可以检查对象是否**真正 “自己拥有” 某属性或者方**法。
...
## 11.3 **prototype的另一种写法**

![image-20221113103123143](C:\Users\LENOVO\AppData\Roaming\Typora\typora-user-images\image-20221113103123143.png)

# 

`对象._proto_`指向`类.prototype`

所以对象可以直接使用类里面的`prototype`的方法
...
![image-20221113104223375](C:\Users\LENOVO\AppData\Roaming\Typora\typora-user-images\image-20221113104223375.png)
...
- +new Date() : 获取当前时间的时间戳
- +new Date('2022-11-1 18:00:00')：获取输入时间的时间戳
...
### 17.3 `Intl.DateTimeFormat`

时间字符串规范化：https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat
...
cdn：https://www.bootcdn.cn/dayjs/
