...
<img src="mark-img/image-20230311202758429.png" alt="image-20230311202758429" style="zoom:67%;" />



先执行第一个宏任务，执行完后再执行第一个微任务，如果微任务里面还有微任务，则不断执行微任务。微任务执行完后再执行下一个宏任务（即进行下一轮循环）
...
new Promise((resolve) => {
    console.log('promise one') // 这个也是宏任务

    resolve(1)

    console.log('promise then') // 这个也是宏任务
}).then((res) => {
    console.log('promise over') // 这个才是微任务
})

console.log('script over')
```

<img src="mark-img/image-20230311204933635.png" alt="image-20230311204933635" style="zoom: 80%;" />
